...
In-paper anchor: `paper/HILBERTS_TENTH_PROBLEM_OVER_Q_PREPRINT.md` (`H10Q_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving undecidability of rational Diophantine solvability through an admissible definability closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`H10Q_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and coding windows do not collapse.
This is the first-failure capture contribution to `M_H10Q`.
...
If near-failure extraction and no-Zeno spacing hold, then `H10Q_G3` prevents hidden drift before rigidity is applied.
