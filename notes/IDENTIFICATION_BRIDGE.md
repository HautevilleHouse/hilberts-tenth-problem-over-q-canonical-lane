...
In-paper anchor: `paper/HILBERTS_TENTH_PROBLEM_OVER_Q_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving undecidability of rational Diophantine solvability through an admissible definability closure architecture`.
...
- endpoint lock: `undecidability_transfer`.
...
2. the endpoint-transfer lock `undecidability_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_H10Q` remains positive.
...
Payload: verify `undecidability_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_H10Q` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `undecidability_transfer`
