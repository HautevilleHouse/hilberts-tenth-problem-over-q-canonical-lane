...
In-paper anchor: `paper/HILBERTS_TENTH_PROBLEM_OVER_Q_PREPRINT.md` (`H10Q_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving undecidability of rational Diophantine solvability through an admissible definability closure architecture`.
...
- projected core: the response sector controlled by `kappa_definability`.
...
`H10Q_G1` closes when `kappa_definability` satisfies the response-floor requirement: projected definability response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_H10Q`.
...
Payload: verify that all quantities used by `kappa_definability` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `kappa_definability` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_definability`, then `H10Q_G1` supplies the projected/protected-core input to `M_H10Q`.

## Current Instantiation

- gate: `H10Q_G1`
- artifact key: `kappa_definability`
