...
## Goal
Separate the rigidity job from the endpoint-transfer job for `proving undecidability of rational Diophantine solvability through an admissible definability closure architecture`.
...
- endpoint-transfer lock: `undecidability_transfer`.
- coherence interface: `eps_coh` remains available to the bridge and final margin.

## Closure Criterion

`H10Q_G4` closes when `rho_rigidity` excludes bad endpoint alternatives: bad decidability countermodels are excluded.
`H10Q_G5` closes when `undecidability_transfer` transfers the surviving endpoint to the intended target class: rigid limit transfers to the undecidable endpoint class.
Together they feed the strict margin `M_H10Q`.
...
The surviving rigid representative is locked to the standard problem-side endpoint by `undecidability_transfer`.

Payload: read this note together with `notes/IDENTIFICATION_BRIDGE.md`.

### Theorem EG4.3 (rigidity/transfer gate closure)
If bad limits are excluded and the endpoint lock is active, then `H10Q_G4` and `H10Q_G5` deliver the boundary object needed by the final margin.

## Current Instantiation

- rigidity gate: `H10Q_G4`
